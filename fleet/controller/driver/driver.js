...
//         if (!token) {
//             return res.redirect('/fleet/sign-in'); // Redirect to login if token is missing
//         }
...
exports.getAllDrivers = async(req, res) => {
    try {
        ...
        res.render('fleet/components/driver/driver', { drivers, error: null });
    } catch (error) {
        if (error.response && error.response.status === 401) {
            return res.redirect('/fleet/sign-in');
        }
...
//         res.redirect('/fleet/manage-driver'); // Redirect to the list of drivers after deletion
...
//         //     }
//         // });
//         res.redirect('/fleet/manage-driver');
//     } catch (error) {
