...
exports.signUpFleet = async(req, res) => {
    ...
    try {
        ...
        return res.redirect('/fleet/manage-fleet');
    } catch (error) {
...
exports.renderFleetSignUp = (req, res) => {
    res.render('fleet/layouts/fleet/new-fleet', { error: null });
};
...
exports.getAllFleets = async(req, res) => {
    try {
        ...
        if (!fleets || fleets.length === 0) {
            return res.render('fleet/components/fleet/fleet', { fleets: [], error: 'No fleets available.' });
        }

        res.render('fleet/components/fleet/fleet', { fleets, error: null });
    } catch (error) {
        if (error.response && error.response.status === 401) {
            return res.redirect('/fleet/sign-in');
        }

        res.render('fleet/components/fleet/fleet', { fleets: [], error: 'Error fetching fleets.' });
    }
};
...
exports.getfleetById = async(req, res) => {
    ...
    try {
        ...
        // console.log(fleet)
        if (!fleet) {
            return res.status(404).render('fleet/components/fleet/view-fleet', { fleet: null, error: 'fleet not found.' });
        }

        res.render('fleet/components/fleet/view-fleet', { fleet, error: null });
    } catch (error) {
        ...
        res.status(500).render('fleet/components/fleet/view-fleet', { fleet: null, error: 'Failed to fetch fleet.' });
    }
};