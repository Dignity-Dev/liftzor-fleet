...
exports.getAllVehicle = async(req, res) => {
    try {
        ...
        if (!vehicle || vehicle.length === 0) {
            return res.render('fleet/vehicle/vehicle', { vehicle: [], error: 'No vehicle available.' });
        }

        res.render('fleet/vehicle/vehicle', { vehicle, error: null });
    } catch (error) {
        if (error.response && error.response.status === 401) {
            return res.redirect('/sign-in');
        }

        res.render('fleet/vehicle/vehicle', { vehicle: [], error: 'Error fetching vehicle.' });
    }
};
...
exports.getvehicleById = async(req, res) => {
    ...
    try {
        ...
        const response = await axios.get(`${process.env.APP_URI}/admin/getOnevehicle`, {
            headers: {
        ...
        if (!vehicle || vehicle.length === 0) {
            ...
            return res.status(404).render('fleet/vehicle/view-vehicle', { vehicle: null, error: 'vehicle not found.' });
        }

        // Render the vehicle details page with the retrieved data
        console.log('vehicle fetched successfully:', vehicle);
        res.render('fleet/vehicle/view-vehicle', { vehicle, error: null });
        ...
        res.status(500).render('fleet/vehicle/view-vehicle', { vehicle: null, error: 'Error fetching vehicle details.' });
    }
};
